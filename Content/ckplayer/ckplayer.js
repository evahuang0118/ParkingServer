/*
	...
	---------------------------------------------------------------------------------------------
	开发说明：
	使用的主要程序语言：javascript(js)及actionscript3.0(as3.0)(as3.0主要用于flashplayer部分的开发，不在该頁面呈现)
	功能：播放视频
...
function ckplayerConfig() {
	return {
		...
		config: {
			...
			previewDefaultLoad: true, //预览图片是否默认加载，优点是鼠标第一次经过进度條即可显示预览图片
			promptSpotTime: false, //提示点文字是否在前面加上对应时间
			buttonMode: {
				player: false, //鼠标在播放器上是否显示可点击形态
				controlBar: false, //鼠标在控制栏上是否显示可点击形态
				timeSchedule: true, //鼠标在时间进度條上是否显示可点击形态
...
!(function() {
	...
	var ckplayer = function(obj) {
		...
		*/
		...
		//全局变量/变量类型：Boolean/功能：设置进度按钮及进度條是否跟着时间变化，该属性主要用来在按下进度按钮时暂停进度按钮移动和进度條的长度变化
		this.isTimeButtonMove = true;
		//全局变量/变量类型：Boolean/功能：进度栏是否有效，如果是直播，则不需要监听时间让进度按钮和进度條变化
	...
	ckplayer.prototype = {
		...
		*/
		resetPlayer: function() {
			...
			if(this.showFace) {
				this.timeProgress(0, 1); //改变时间进度條宽
				this.changeLoad(0);
		...
		interFace: function() {
			...
			*/
			var controlBarBgID = 'controlbgbar' + randomS, //控制栏背景
				...
				timeProgressBgID = 'timeprogressbg' + randomS, //播放进度條背景
				loadProgressID = 'loadprogress' + randomS, //加载进度條
				timeProgressID = 'timeprogress' + randomS, //播放进度條
				timeBOBGID = 'timebobg' + randomS, //播放进度按钮容器，该元素为一个透明覆盖在播放进度條上
		...
		*/
		...
		/*
			...
			格式化时分秒
			seconds:Int：秒数
			ishours:Boolean：是否显示小时，如果设置成false，则会显示如80:20，表示1小时20分鐘20秒
		*/
