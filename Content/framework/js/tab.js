    ...
    Tab.prototype.tabAdd = function (data) {
            ...
            if (_config.maxSetting !== 'undefined') {
                ...
                if (typeof _config.maxSetting === 'number') {
                    if (currentTabCount === _config.maxSetting) {
                        layer.msg('為了系统的流暢度，只能同时打開' + _config.maxSetting + '個選項卡。');
                        return;
                    }
                }
                if (typeof _config.maxSetting === 'object') {
                    var max = _config.maxSetting.max || 8;
                    var msg = _config.maxSetting.tipMsg || '為了系统的流畅度，只能同時打開' + max + '個選項卡。';
        ...
        if (_config.contextMenu) {
            ...
            ELEM.titleBox.find('li').on('contextmenu', function (e) {
                ...
                //判断，如果存在右键菜单的div，则移除，保存頁面上只存在一个
                ...
                //ul += '<li data-target="closeCurrent">關閉當前</li>';
                //ul += '<li data-target="closeOther">關閉其他</li>';
                //ul += '<li data-target="closeAll">关闭全部</li>';

                //ul += '<li data-target="refresh" title="刷新当前选项卡"><i class="fa fa-refresh" aria-hidden="true"></i> 刷新</li>';
                ul += '<li data-target="closeCurrent" title="關閉當前選項卡"><i class="fa fa-close" aria-hidden="true"></i> 關閉當前</li>';
                ul += '<li data-target="closeOther" title="關閉其他選項卡"><i class="fa fa-window-close-o" aria-hidden="true"></i> 關閉其他</li>';
                ul += '<li data-target="closeAll" title="關閉全部選項卡"><i class="fa fa-window-close-o" aria-hidden="true"></i> 全部關閉</li>';
